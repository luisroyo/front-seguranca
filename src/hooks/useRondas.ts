...
export const useRondas = (filters: RondasFilters = {}) => {
  ...
  return {
    ...
    // Filtros úteis
    rondasAtivas: rondasData?.data.filter(r => r.status === 'em_andamento') || [],
    rondasConcluidas: rondasData?.data.filter(r => r.status === 'concluida') || [],
    rondasPendentes: rondasData?.data.filter(r => r.status === 'pendente') || []
  }
}
